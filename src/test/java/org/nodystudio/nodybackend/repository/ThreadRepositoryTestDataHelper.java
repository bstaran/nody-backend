...
public class ThreadRepositoryTestDataHelper {
   ...
   *
   * <p>
   * JPA Auditing으로 인해 자동으로 설정된 생성일시를 테스트 요구사항에 맞게 수정합니다.
   * 주로 시간 순서에 따른 정렬 테스트에서 사용됩니다.
   * </p>
   *
   ...
   *
   * <p>
   * 생성된 테스트 엔티티들을 타입별로 분류하여 저장하며,
   * 테스트에서 생성된 데이터에 쉽게 접근할 수 있도록 도와줍니다.
   * </p>
   *
   * <h3>사용 예시:</h3>
   *
   ...
   *
   * <p>
   * 플루언트 API를 통해 User, Log, Thread 엔티티를 단계적으로 생성하고
   * 자동으로 데이터베이스에 persist합니다. 각 엔티티는 커스터마이저 함수를 통해 테스트 요구사항에 맞게 설정할 수 있습니다.
   * </p>
   *
   * <h3>워크플로우:</h3>
   * <ol>
   * <li><strong>설정 단계</strong>: {@code withTimestamps()},
   * {@code withCustomUser()}, {@code withCustomLog()}</li>
   ...
   * <li><strong>Log</strong>: latitude=37.5665, longitude=126.9780 (사용자는 명시적 지정
   * 필수)</li>
  ...
  public static class TestDataBuilder {
     ...
     *
     * <p>
     * 이 메소드는 {@code buildUsersAndLogs()} 호출 후에만 유효한 데이터를 반환합니다.
     * 주로 중간 단계에서 생성된 User나 Log 엔티티에 접근할 때 사용됩니다.
     * </p>
     *
     ...
     *
     * <p>
     * 이 설정은 {@code finalizeWithTimestamps()} 호출 시 스레드들의 생성일시를
     * 자동으로 업데이트하는 데 사용됩니다. 각 스레드는 baseTime에서 시작하여 intervalMinutes 간격으로 생성일이 설정됩니다.
     * </p>
     *
     ...
     *
     * <p>
     * 사용자별 고유 필드(email, nickname, socialId)는 반드시 설정해야 하며,
     * provider와 role은 자동으로 기본값이 설정됩니다.
     * </p>
     *
     ...
     *
     * <p>
     * 로그의 content와 user는 반드시 설정해야 하며, 위치 정보만 자동으로 기본값이 설정됩니다.
     * </p>
     *
     ...
     *
     * <p>
     * 이 메소드는 {@code addThread()} 메소드를 사용하기 전에 반드시 호출해야 합니다.
     * 사용자와 로그를 먼저 생성하여 스레드 생성 시 참조할 수 있도록 합니다.
     * </p>
     *
     * <p>
     * 중복 호출해도 안전하며, 이미 생성된 경우 아무 작업을 수행하지 않습니다.
     * </p>
     *
     * @return 이 빌더 인스턴스
     *
     ...
     *
     * <p>
     * 이 메소드는 각 스레드를 생성하자마자 즉시 데이터베이스에 저장하므로,
     * 다음 스레드를 생성할 때 이전 스레드를 참조할 수 있습니다.
     * </p>
     *
     * <p>
     * 사용자는 반드시 명시적으로 지정해야 합니다.
     * </p>
     *
     ...
     *
     * <p>
     * 이 메소드 호출 후에는 새로운 스레드를 추가할 수 없습니다.
     * </p>
     *

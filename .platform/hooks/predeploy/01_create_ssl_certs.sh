...
echo "AWS 리전을 확인합니다 (IMDSv2 사용)."
TOKEN=$(curl -s -X PUT "http://169.254.169.254/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 21600")
AWS_REGION=$(curl -s -H "X-aws-ec2-metadata-token: $TOKEN" http://169.254.169.254/latest/meta-data/placement/region)
if [ -z "$AWS_REGION" ]; then
    echo "오류: 인스턴스 메타데이터에서 AWS 리전을 확인할 수 없습니다. (IMDSv2)"
    exit 1
fi
...
echo "Parameter Store에서 Cloudflare Origin Pull CA 인증서를 가져옵니다 (파라미터: $CA_PARAM_NAME)..."
...
if [ -z "$CA_CONTENT" ]; then
    echo "오류: Parameter Store에서 CA 인증서를 가져오지 못했거나 파라미터 값이 비어있습니다."
    echo "'$AWS_REGION' 리전에 '$CA_PARAM_NAME' 파라미터가 존재하는지, 그리고 인스턴스에 권한이 있는지 확인하세요."
    exit 1
fi

echo "Cloudflare Origin Pull CA 인증서 파일을 생성합니다..."
mkdir -p /etc/pki/tls/certs
echo "$CA_CONTENT" > /etc/pki/tls/certs/cloudflare-ca.pem
echo "CA 인증서 파일이 /etc/pki/tls/certs/cloudflare-ca.pem 경로에 생성되었습니다."
